...
function SaveOrUpdateTemplateConfig($courseCode, $completeDate, $defaultTemplate, $convocationDate, $convocationPlace, $transcriptBack, $duration)
{
    global $lms_link;
    ...
    if ($result_check->num_rows > 0) {
        ...
        $sql_update = "UPDATE `certificate_course_end_date` SET `CompleteDate` = ?, `defaultTemplate` = ?, `convocation_date` = ?, `convocation_place` = ?, `transcript_back` = ?, `duration` = ? WHERE `CourseCode` = ?";
        $stmt_update = $lms_link->prepare($sql_update);
        $stmt_update->bind_param("sssssss", $completeDate, $defaultTemplate, $convocationDate, $convocationPlace, $transcriptBack, $duration, $courseCode);
        ...
        // Insert a new record
        $sql_insert = "INSERT INTO `certificate_course_end_date` (`CourseCode`, `CompleteDate`, `defaultTemplate`, `convocation_date`, `convocation_place`, `transcript_back`, `duration`) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt_insert = $lms_link->prepare($sql_insert);
        $stmt_insert->bind_param("sssssss", $courseCode, $completeDate, $convocationDate, $convocationPlace, $transcriptBack, $defaultTemplate, $duration);
