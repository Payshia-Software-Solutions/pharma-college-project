...
try {
    ...
    $response = $client->request(
        'GET',
        $_ENV['SERVER_URL'] . "/delivery_orders?indexNumber={$loggedUser}&receivedStatus=false"
    );
    ...
    if (
        ...
        $responseData['error'] === 'No delivery orders found for the given index number'
    ) {
        $notReceivedOrders = [];
    } else {
...
$word_pallet_total_words     = 0;
$word_pallet_correct_count   = 0;
$word_pallet_incorrect_count = 0;
$word_pallet_grade           = 0;

try {
    $studentEntries = $client
        ->request('GET', $_ENV['SERVER_URL'] . '/en-word-submissions/student-grades/' . $loggedUser)
        ->toArray();
...
include './components/ceremony-number.php';
...
<div class="row mt-4">
    <div class="col-12">
        <?php if (!empty($notReceivedOrders)) { ?>
            <div class="alert alert-warning">
                <p class="mb-0">
                    Please click the "Received" button if you have received your order. This will help us update the
                    status of your order promptly. Thank you!<br>
                    ඔබගේ ඇනවුම ලබා ගත්තා නම් "Received" බටනය ඔබා එය තත්වය යාවත්කාලීන කිරීමේදි අපට උදව් කරන්න. ස්තූතියි!
                </p>
            </div>

            <?php foreach ($notReceivedOrders as $selectedArray) { ?>
...
<?php include './components/play-modules.php'; ?>

<div class="border-bottom my-3"></div>

<?php include './components/other-modules.php'; ?>