...
function PrintShippingLabel(ref_id) {
  ...
  if (trackingNumber != "" && codAmount != "" && packageWeight > 0) {
    ...
    var printWindow = window.open(
      "report-viewer/shipping-label?ref_id=" + encodeURIComponent(ref_id),
      "_blank"
    );
...
function GetOrderSummary(studentBatch = 0, orderType = 0) {
  OpenPopup();
  document.getElementById("loading-popup").innerHTML = InnerLoader;

  function fetch_data() {
    $.ajax({
      url: "assets/content/lms-management/orders/order-summary.php",
      method: "POST",
      data: {
        LoggedUser: LoggedUser,
        UserLevel: UserLevel,
