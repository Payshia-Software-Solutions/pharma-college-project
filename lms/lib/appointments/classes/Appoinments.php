...
{
    protected $db;
    protected $table_name = "hunter_saveanswer";
    protected $lastError;
    ...
    function savedAnswersByUserMedicine($medicineId, $userId)
    {
        try {
            $query = "SELECT * FROM " . $this->table_name . " WHERE medicine_id = :medicine_id AND index_number LIKE :index_number";
            $stmt = $this->db->prepare($query); // Use the PDO connection
            $stmt->bindParam(':medicine_id', $medicineId, PDO::PARAM_STR);
            $stmt->bindParam(':index_number', $userId, PDO::PARAM_STR);
            $stmt->execute();
