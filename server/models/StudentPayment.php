...
class StudentPaymentNew
{
    ...
    {
        $stmt = $this->pdo->prepare("INSERT INTO `student_payment` (`receipt_number`, `course_code`, `student_id`, `paid_amount`, `discount_amount`, `payment_status`, `payment_type`, `paid_date`, `created_at`, `created_by`) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $data['receipt_number'],
            $data['course_code'],
