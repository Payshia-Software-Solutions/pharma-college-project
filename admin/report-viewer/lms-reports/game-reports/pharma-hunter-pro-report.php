...
// Get Hunter Pro Medicines
$response = $client->request('GET', 'https://api.pharmacollege.lk/hp-course-medicine/course/' . $studentBatch);
$hunterProMedicinesByCourse = $response->toArray();

// Get Saved Counts
$response = $client->request('GET', 'https://api.pharmacollege.lk/hp-saved-answers-counts/');
$savedCounts = $response->toArray();

$attemptPerMedicine = 10;
$medicineCount = count($hunterProMedicinesByCourse);

$generateDate = new DateTime();
$reportDate = $generateDate->format('d/m/Y H:i:s');

$pageTitle = "Pharma Hunter Pro Assessment Report - " . $studentBatch;;
$reportTitle = "Pharma Hunter Pro Assessment Report";
?>
