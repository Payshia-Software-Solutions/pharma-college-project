...
$(document).ready(function () {
  ...
  $(document).on("click", function (event) {
    if (
      !$(event.target).closest(".profile-container, .fa-user.top-icon").length
    ) {
  ...
  $(document).on("click", function (event) {
    if (
      !$(event.target).closest(".notification-container, .fa-bell.top-icon")
        .length
    ) {
  ...
  $(".nav-link").each(function () {
    ...
    if (currentUrl.endsWith(fileName)) {
      ...
      if (parentSubmenu.length > 0) {
        parentSubmenu.show();
        var parentCollapseIcon = parentSubmenu
          .prev(".nav-link")
          .find(".collapse-icon");
...
function toggleBackgrounds() {
  ...
  const loadingPopRight = document.getElementById(
    "loading-popup-content-right"
  );
