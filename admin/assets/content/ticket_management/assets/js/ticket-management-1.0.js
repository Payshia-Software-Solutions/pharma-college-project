...
function OpenTicket(ticketId) {
  OpenPopupRight();
  document.getElementById("loading-popup-right").innerHTML = InnerLoader;
  ...
  function fetch_data() {
    $.ajax({
      ...
      success: function (data) {
        ...
        forceTicketAssignmentUpdate(LoggedUser, ticketId, (CloseState = 0));
        forceChangeTicketStatus(ticketId, 4);
      },
    });
  }
  fetch_data();
...
function forceChangeTicketStatus(ticketId, ticketStatus) {
  function fetch_data() {
    showOverlay();
    $.ajax({
      url: "./assets/content/ticket_management/change-ticket-status.php",
...
function forceTicketAssignmentUpdate(userName, ticketId, CloseState = 0) {
  function fetch_data() {
    $.ajax({
      url: "./assets/content/ticket_management/assign-ticket.php",
      method: "POST",
